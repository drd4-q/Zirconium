...
if [ ! -f disk2.img ]; then
    ...
    dd if=/dev/zero of=disk2.img bs=1M count=64 2>/dev/null
    # Format as FAT16 using mtools (no -F flag which forces FAT32)
    # For 64MB, mformat auto-detects FAT16
    mformat -i disk2.img ::
    # Create directories
    mmd -i disk2.img ::/test
    # Create test files
    echo "Hello from Zirconium!" | mcopy -i disk2.img - ::/hello.txt
    echo "Test file in subdirectory" | mcopy -i disk2.img - ::/test/data.txt
