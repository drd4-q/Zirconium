...
pub fn init() void {
    ...
    port.serialWrite("[NET] IP: 10.0.2.15 GW: 10.0.2.2\n");
}
...
pub fn sendFrame(dst: [6]u8, eth_type: u16, payload: []const u8) void {
    ...
    send_buf[12] = @intCast(eth_type >> 8);
    send_buf[13] = @intCast(eth_type & 0xFF);
    var i: usize = 0;
...
pub fn printIpSerial(ip: [4]u8) void {
    port.serialWriteHex(ip[0]);
    port.serialWrite(".");
    port.serialWriteHex(ip[1]);
    port.serialWrite(".");
    port.serialWriteHex(ip[2]);
    port.serialWrite(".");
    port.serialWriteHex(ip[3]);
}
